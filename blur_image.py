...
def read_grid(file_path):
    ...
    for line in myfile:
        mylist.append(csv_line_to_list(line))
    return mylist
...
def average_of_surrounding(pixel_grid, i, j):
    ...
    """
    pixel_sum = 0
    for row in range(i - 1, i + 2):  # Rows
        for column in range(j - 1, j + 2):  # Columns
...
def blur(pixel_grid):
    ...
    blur_matrix = []  # Create empty matrix
    for row in range(len(pixel_grid)):  # Rows
        new_list = []  # List for new rows
        for column in range(len(pixel_grid[0])):  # Columns
            new_list.append(average_of_surrounding(pixel_grid, row, column))
        blur_matrix.append(new_list)
    return blur_matrix
...
print("Welcome to the Image Blurring program!")
